...
/** Per-step player update: apply input to movement/firing; coast on ice (C16). */
...
export function damagePlayer(world: World): void {
  ...
  p.shieldUntil = 0;
  p.slide = null; // R4: respawn never carries momentum (T-TER-6)
  if (p.lives > 0) {
    p.pos = { ...p.spawnPos };
    p.dir = Direction.UP;
