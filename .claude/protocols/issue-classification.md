...
### Scope 上限(ADR-010 / 2026-06-01)

四分类(A/B/C/D)覆盖 **EL 执行层**的 issue 性质。**product-collaboration 层**(PM/AM/team 协调 / 决策 / 交付任务)**在四分类 scope 之外** —— 不为其加第 5 类 / 不加 `type:coordination` / `type:decision` / `type:deliverable` / 统一 `type:task`。此类事项由项目自管(项目自己 repo 本地 label)。本 scope 上限同样约束本协议未来形式化(ADR-009)。详见 ADR-010。

**Dogfood 实证(cross-family):** wesado(GitHub #28 / 第 2 个跨家族实践)阶段 1 启动前 6 个团队协调事项无一 fit A/B/C/D,实证四分类的 EL 聚焦边界 + product-collaboration scope 上限。计入本协议 dogfood-judge 的 cross-family 实证维度(wesado = product-collaboration gap 第 1 项目 / Re-eval trigger ≥3 项目)。
...
| 2026-06-01 | v0 (Proposed) | 加 §Scope 上限段(ADR-010 / product-collaboration out-of-scope / 约束本协议未来形式化)+ wesado(#28)cross-family dogfood 实证记录 |
